...
// connectionOnce connects to the groselha server and blocks
func (c *Client) connectionOnce(ctx context.Context) (connected bool, err error) {
	...
	d := websocket.Dialer{
		...
		Subprotocols:     []string{grshare.ProtocolVersion},
	...
	defer sshConn.Close()
	// groselha client handshake (reverse of server handshake)
	// send configuration

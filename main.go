...
var help = `
  ...
  Version: ` + grshare.BuildVersion + ` (` + runtime.Version() + `)
...
func main() {
	...
	if *version || *v {
		fmt.Println(grshare.BuildVersion)
		os.Exit(0)
	}
...
var commonHelp = `
  ...
  Version:
    ` + grshare.BuildVersion + ` (` + runtime.Version() + `)
...
func generatePidFile() {
	...
	if err := ioutil.WriteFile("groselha.pid", pid, 0644); err != nil {
		log.Fatal(err)
	}
}
...
func server(args []string) {
	...
	if config.KeySeed == "" {
		config.KeySeed = os.Getenv("groselha_KEY")
	}
	s, err := grserver.NewServer(config)
	if err != nil {
		log.Fatal(err)
	}
	s.Debug = *verbose
...
func client(args []string) {
	...
	config := grclient.Config{Headers: http.Header{}}
	...
	//ready
	c, err := grclient.NewClient(&config)
	if err != nil {
		log.Fatal(err)
	}
	c.Debug = *verbose

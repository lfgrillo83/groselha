...
// handleClientHandler is the main http websocket handler for the groselha server
func (s *Server) handleClientHandler(w http.ResponseWriter, r *http.Request) {
	//websockets upgrade AND has groselha prefix
	upgrade := strings.ToLower(r.Header.Get("Upgrade"))
	protocol := r.Header.Get("Sec-WebSocket-Protocol")
	if upgrade == "websocket" {
		if protocol == grshare.ProtocolVersion {
			s.handleWebsocket(w, r)
			return
		}
		//print into server logs and silently fall-through
		s.Infof("ignored client connection using protocol '%s', expected '%s'",
			protocol, grshare.ProtocolVersion)
	}
	...
	case "/health":
		...
		return
	case "/version":
		w.Write([]byte(grshare.BuildVersion))
		return
	}
	//missing :O
	w.WriteHeader(404)
...
func (s *Server) handleWebsocket(w http.ResponseWriter, req *http.Request) {
	...
	// groselha server handshake (reverse of client handshake)
	...
	if c.Version != grshare.BuildVersion {
		v := c.Version
		if v == "" {
			v = "<unknown>"
		}
		l.Infof("Client version (%s) differs from server version (%s)",
			v, grshare.BuildVersion)
	}
	//validate remotes

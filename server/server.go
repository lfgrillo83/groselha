...
// Config is the configuration for the groselha service
type Config struct {
	KeySeed   string
	AuthFile  string
	Auth      string
...
// Server respresent a groselha service
type Server struct {
	*cio.Logger
	config       *Config
	fingerprint  string
...
// NewServer creates and returns a new groselha server
func NewServer(c *Config) (*Server, error) {
	...
	server.sshConfig = &ssh.ServerConfig{
		ServerVersion:    "SSH-" + grshare.ProtocolVersion + "-server",
...
// Run is responsible for starting the groselha service.
